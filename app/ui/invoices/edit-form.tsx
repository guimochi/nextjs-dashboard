...
import { State, updateInvoice, updateInvoiceBis } from "@/app/lib/actions";
...
export default function EditInvoiceForm({
  ...
  const [state, dispatch] = useFormState(
    (prevState: State, formData: FormData) =>
      updateInvoiceBis(invoice.id, prevState, formData),
    initialState,
  );
  ...
  return (
    <form action={dispatch} aria-describedby={"form-error"}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div id={"form-error"} className={"text-2xl text-red-500 mt-2"}>
          {state.message}
        </div>
      </div>
